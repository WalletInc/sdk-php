...
```php
fetchFreePlan(): \OpenAPI\Client\Model\SubscriptionPlan
```
...
try {
    $result = $apiInstance->fetchFreePlan();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling IndustriesApi->fetchFreePlan: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\OpenAPI\Client\Model\SubscriptionPlan**](../Model/SubscriptionPlan.md)
