...
class WTWalletConfigurationSaveWalletRecord implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setSmsOptInSourceId($sms_opt_in_source_id)
    {
        if (!is_null($sms_opt_in_source_id) && (mb_strlen($sms_opt_in_source_id) > 10)) {
            throw new \InvalidArgumentException('invalid length for $sms_opt_in_source_id when calling WTWalletConfigurationSaveWalletRecord., must be smaller than or equal to 10.');
        }
        if (!is_null($sms_opt_in_source_id) && (mb_strlen($sms_opt_in_source_id) < 10)) {
            throw new \InvalidArgumentException('invalid length for $sms_opt_in_source_id when calling WTWalletConfigurationSaveWalletRecord., must be bigger than or equal to 10.');
        }
        if (!is_null($sms_opt_in_source_id) && (!preg_match("/^[a-zA-Z0-9]+$/", $sms_opt_in_source_id))) {
            throw new \InvalidArgumentException("invalid value for $sms_opt_in_source_id when calling WTWalletConfigurationSaveWalletRecord., must conform to the pattern /^[a-zA-Z0-9]+$/.");
        }
